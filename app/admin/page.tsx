...
export default function AdminDashboard() {
  ...
  const exportCSV = () => {
    ...
    // CSV extendido con datos de comunidades y KPIs agregados
    const header = [
      'community_id',
      'community_code',
      'community_name',
    ...
    const rows = dashboard.communities.map(c => [
      c.id,
      c.code,
      c.name,
      c.color,
      c.sessions_count,
      dashboard.kpis.avg_delta_valence.toFixed(2),
      dashboard.kpis.avg_delta_energy.toFixed(2),
      (dashboard.kpis.after_response_rate * 100).toFixed(1) + '%',
      dashboard.kpis.today,
      dashboard.kpis.week,
      dashboard.kpis.median_duration
    ].map(String));
    ...
    a.href = url;
    a.download = `dashboard_admin_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
